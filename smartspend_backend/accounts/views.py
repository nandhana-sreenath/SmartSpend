...
#     # Frame 1
#     settings.month = request.data.get('month') or settings.month
#     settings.year = request.data.get('year') or settings.year
...
#     # Get user's budget settings for the selected month/year
#     settings = BudgetSettings.objects.filter(user=user, month=month, year=year).first()
#     net_income = settings.net_income if settings and settings.net_income else 0
...
# @api_view(['GET'])
# def monthly_finance_view(request):
#     email = request.GET.get('email')
...
#     # Step 1: Get all budget settings for this user
#     settings = BudgetSettings.objects.filter(user=user)

#     # Step 2: Get all expenses for this user
#     expenses = Expense.objects.filter(user=user)

#     # Step 3: Aggregate income and spent by month/year
#     monthly_data = defaultdict(lambda: {'income': 0, 'spent': 0})

#     for s in settings:
#         if s.month and s.year:
#             key = f"{s.year}-{s.month}"
#             monthly_data[key]['income'] += float(s.net_income or 0)

#     for e in expenses:
#         if e.date:
#             key = f"{e.date.year}-{e.date.month}"
#             monthly_data[key]['spent'] += float(e.amount or 0)
...
def budget_settings_view(request):
    ...
    net_income = request.data.get('net_income')
    ...
    settings, created = BudgetSettings.objects.get_or_create(
        user=user,
        month=month,
        year=year,
    ...
    if not created:
        settings.net_income = net_income
        settings.save()
